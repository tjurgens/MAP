...
%       bandwidth (BW). (In this example: N = 30)
%    
...
Verbatim = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
lower_cutoff_frequency_hz = 233;%100;%70;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%TT
y_response = zeros(length(x_response),channels); %preallocation due to speed
%%TT
 for FreqChanNr = 1:channels,
     
     
    %y = gammatone_r(x, CenterFreq(FreqChanNr), BW, fs);   % gammatone
    %bandpass filter old
	%y = haircell(real(x_response(FreqChanNr,:)'), fs);    % haircell model 
    y_response(:,FreqChanNr) = haircell(real(x_response(FreqChanNr,:)'), fs);    % haircell model 
    %y = adapt(y, fs);     % adaptation loops
    y_response(:,FreqChanNr) = adapt(y_response(:,FreqChanNr), fs);     % adaptation loops
    %y = 10*log10(y+eps);
 end
    ...
    if ShowWaitBar,
        waitbar(FreqChanNr/channels,h);                  % refresh display of progress
    end
...
%%-------------------------------------------------------------------------
%%
%%  Copyright (C) 1998-2006     University of Oldenburg, Medical Physics
%%  
