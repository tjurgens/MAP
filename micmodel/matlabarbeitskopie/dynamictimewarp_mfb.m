function distance = dynamictimewarp_mfb(template, testsignal, distancemeasure)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%   Word Recognition
%
%  (c) Tim Jürgens, Medizinische Physik, Sept.2006
%
%  usage: distance = dynamictimewarp_mfb(template, testsignal)
%
%   calculates distance with DTW 
%   adapted to time-frequency-modfreq matrices like output of PEMO
%   IR(a,b){c}: a:frequency channels
%               b:timesteps
%               c:modulation channels
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global distancestatisticvec
if nargin < 2,
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
template = cell2matrix3(template);         %make 3-d-matrix out of cell-2-d-matrices
testsignal = cell2matrix3(testsignal);
distancematrix = zeros(size(template,2),size(testsignal,2)); %preallocation due to speed
...
if strcmp(distancemeasure,'Euklid')
    for i = 1:size(template,2)
        for j = 1:size(testsignal,2)
            distancematrix(i,j) = sqrt(sum(sum((template(:,i,:)-testsignal(:,j,:)).^2)));
        end
        if ShowWaitBar,
    ...
    %herausgenommen am 19.6.07 um distancemass zu verändern
    %%%%% END EUCLIDIAN %%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% for i = 1:size(template,2)
%     for j = 1:size(testsignal,2)
%          distancestatisticvec_4D(:,:,i,j) = ...
%              template(:,i,:)-testsignal(:,j,:);
%     end
% end
% if length(distancestatisticvec)>1
%     distancestatisticvec = distancestatisticvec+(hist(distancestatisticvec_4D(:),[-300:0.5:300]));
% else
%     distancestatisticvec = hist(distancestatisticvec_4D(:),[-300:0.5:300]);
% end
% figure, hist(distancestatisticvec(:),1000);
...
% distancematrix = zeros(length(template{1}(1,:)),length(testsignal{1}(1,:))); %preallocation due to speed
% 
% for i = 1:length(template{1}(1,:))
%     for j = 1:length(testsignal{1}(1,:))
%         distancematrix(i,j) = 0;
%         for mod_freq = 2:length(template)    %loop for modulation frequencies  %1!!
%             for f = 1:length(template{1}(:,1))  %loop for frequencies
%                 distancematrix(i,j) = distancematrix(i,j)+(template{mod_freq}(f,i)-testsignal{mod_freq}(f,j)).^2;
%             end
%         end
...
if Make_Kerf_on_distancematrix,
    alpha = 1.5;
    kerf_matrix = make_kerf(alpha,length(template(1,:)),length(testsignal(1,:)));
    distancematrix = distancematrix.*kerf_matrix;
end
% %plot distancematrix
% if Verbatim,
%    figure;
...
%%NORMIERUNG: NEU JT 2009
%distancematrix = distancematrix-min(min(distancematrix));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (i <= size(template,2) || j <= size(testsignal,2))
    ...
    elseif (i == 1)
        ...
        g(i,j) = min([g(i,j-1) + distancematrix(i,j), g(i-1,j-1) + 2*distancematrix(i,j), g(i-1,j) + distancematrix(i,j)]);
    end
end
...
%save('distance.mat','distancematrix','g');
%%%%%%%%%%%%%%%%%% Finding Warppath back %%%%%%%%%%%%%%%%%%%%%%%%%%%
if FindWayBack,
    warppath.template = zeros(1,(size(template,2))+size(testsignal,2));   %you don't know how long the warppath is
    warppath.test = zeros(1,(size(template,2))+size(testsignal,2));
    warppath.template(end) = size(template,2);
    warppath.test(end) = size(testsignal,2);
