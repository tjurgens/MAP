function [ UNIQUEpars, SHAREDpars ] = getDefaultParams(  )
...
for nn = 0:SHAREDpars.NumBands-1 %Channels are indexed from zero!
    ...
    eval(['SHAREDpars.Band_' num2str(nn) '_MOCthreshold_dBspl = 250;']);
end
...
for nn = 0:SHAREDpars.NumBands-1 %Channels are indexed from zero!    
    eval(['UNIQUEpars.Band_' num2str(nn) '_InstantaneousCmpThreshold_dBspl  = 650;']);
